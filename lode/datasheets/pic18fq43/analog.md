# PIC18F27/47/57Q43 Analog Peripherals
...
`DACx_output = (VREF+ - VREF-) * DACxR[7:0] / 256 + VREF-`

### Registers

| Register | Bits | Description |
|----------|------|-------------|
| DAC1CON | EN,OE[1:0],PSS[1:0],NSS | Enable, output enables, source selects |
| DAC1DATL | DAC1R[7:0] | Output level 0-255 |
...
|-----|------|-------------|
| 7 | EN | 1=DAC enabled |
| 5:4 | OE[1:0] | Output enable (see table below) |
| 3:2 | PSS[1:0] | Positive reference select |
| 0 | NSS | Negative reference select |

### OE[1:0] — Output Enable

| OE | DAC Outputs |
|----|-----------|
| 00 | DACxOUT disabled |
| 01 | DACxOUT enabled on pin RB7 only |
| 10 | DACxOUT enabled on pin RA2 only |
| 11 | DACxOUT disabled |

### PSS[1:0] — Positive Reference
...
`TMEAS = (ADC_MEAS * Gain) / 256 + Offset`
...
|-----|------|---------|
| 7 | EN | 1=enabled |
| 5 | OUT | 1=VDD<=trip (voltage < selected limit), 0=VDD>=trip |
| 4 | RDY | 1=band gap stable |
| 1 | INTH | 1=interrupt on VDD>=trip (rising) |
| 0 | INTL | 1=interrupt on VDD<=trip (falling) |

Disable (EN=0) before changing INTH/INTL/SEL to avoid false events. SEL voltages per device Electrical Specifications. Active in Sleep.

---

## ACT — Active Clock Tuning (Q43 Unique)

The Q43 family includes an Active Clock Tuning (ACT) module not present on K42. ACT allows runtime trimming of the HFINTOSC oscillator to improve clock accuracy. The ACT module uses a reference clock source to measure and adjust the HFINTOSC frequency.

### ACT Trigger in DMA
The ACT interrupt (vector 0x0B in the DMA trigger table) can be used as a DMA start or abort source, enabling automated clock calibration without CPU intervention.