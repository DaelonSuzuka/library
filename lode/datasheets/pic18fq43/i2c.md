# PIC18F27/47/57Q43 — I2C Module

## Overview

Single I2C instance (**I2C1**). Supports Host, Client, and Multi-Host modes with 7-bit and 10-bit addressing. SDA and SCL are open-drain bidirectional lines requiring external pull-ups.
...
Enable the module with I2CxCON0.EN = 1. SDA/SCL must be open-drain (clear TRIS, set ODCON). Both PPS input and output must select the same pin.

## Registers

| Register | Description |
|----------|-------------|
| I2C1CON0 | Control 0: EN, RSEN, S, CSTR, MDR, MODE[2:0] |
| I2C1CON1 | Control 1: ACKCNT, ACKDT, ACKSTAT, ACKT, RXO, TXU, CSD |
| I2C1CON2 | Control 2: ACNT, GCEN, FME, ADB, SDAHT[1:0], BFRET[1:0] |
| I2C1CLK | Clock source selection: CLK[3:0] |
| I2C1BTO | Bus timeout source: BTO[2:0] |
| I2C1STAT0 | Status 0: BFRE, SMA, MMA, R, D |
| I2C1STAT1 | Status 1: TXWE, TXBE, RXRE, CLRBF, RXBF |
| I2C1ERR | Error: BTOIF, BCLIF, NACKIF, BTOIE, BCLIE, NACKIE |
| I2C1PIR | Interrupt flags: CNTIF, ACKTIF, WRIF, ADRIF, PCIF, RSCIF, SCIF |
| I2C1PIE | Interrupt/hold enables: CNTIE, ACKTIE, WRIE, ADRIE, PCIE, RSCIE, SCIE |
| I2C1CNT | Byte count: CNT[7:0] |
| I2C1ADR0 | Client address 0 |
| I2C1ADR1 | Client address 1 / 10-bit high address |
| I2C1ADR2 | Client address 2 / 10-bit low address / mask |
| I2C1ADR3 | Client address 3 / 10-bit high mask |
| I2C1ADB0 | Address data buffer 0 |
| I2C1ADB1 | Address data buffer 1 |
| I2C1TXB | Transmit buffer (write-only) |
| I2C1RXB | Receive buffer (read-only) |

### I2C1CON0 — Control Register 0 (reset 0x00)
...
### I2C1CON1 — Control Register 1 (reset 0x04)
...
### I2C1CON2 — Control Register 2 (reset 0x00)
...
### I2C1STAT0 — Status Register 0 (reset 0x00)

| Bit | Name | Description |
|-----|------|-------------|
| 7 | BFRE | Bus free (1=idle; requires valid I2C1CLK source) |
...
### I2C1CLK — Clock Source Selection
...
When the selected timeout source goes high, the I2C module resets. In client mode with SMA=1, the module resets immediately and sets BTOIF. In host mode with MMA=1, the module attempts a Stop condition then clears MMA and sets BTOIF.

### I2C1ERR — Error Register (reset 0x00)
...
Enabled error flags are OR'd to produce PIRx.I2C1EIF.

## Addressing Modes

### 7-bit Client Addressing (MODE=000/001/110/111)
- MODE=000: Four independent 7-bit addresses in ADR0-ADR3; LSb of received byte ignored during comparison
...
### Host Addressing
- MODE=100 (7-bit): Address in I2C1ADB1[7:1], R/W in I2C1ADB1[0] (ABD=0); or address byte in I2C1TXB (ABD=1)
- MODE=101 (10-bit): High address in I2C1ADB1, low in I2C1ADB0 (ABD=0); or through I2C1TXB (ABD=1). Read requires Restart.
...
|-----------|--------------|----------------|
| RXBF=1 & SMA=1 | 7th SCL falling edge | Read I2C1RXB |
| TXBE=1 & CNT!=0 & SMA=1 | 8th SCL falling edge | Write I2C1TXB |
...
1. Load I2C1CNT with byte count
2. Load address (I2C1ADB0/1 if ABD=0, or I2C1TXB if ABD=1)
...
7. If ACK: data transfer proceeds, I2C1CNT decrements
8. I2C1CNT=0 -> CNTIF set, Stop (or Restart if RSEN=1)

For host receive: set ACKDT=0 (ACK during transfer), ACKCNT=1 (NACK at end).

## Interrupt Flags (I2C1PIR) and Enables (I2C1PIE)

| Bit | Flag | Set Condition | Enable | Effect when enabled |
|-----|------|--------------|--------|---------------------|
| 7 | CNTIF | I2C1CNT=0 at 9th SCL falling edge | CNTIE | — |
...
All enabled PIR flags OR'd -> PIRx.I2C1IF (generic interrupt).
All enabled ERR flags OR'd -> PIRx.I2C1EIF (error interrupt).

Interrupt vector numbers: I2C1=25, I2C1E=26, I2C1RX=23, I2C1TX=24.
...
- **I2C1TXB**: Write transmit data; TXBE=1 when empty. Write when TXBE=0 -> TXWE error.
- **I2C1RXB**: Read received data; RXBF=1 when full. Read when RXBF=0 -> RXRE error.
- **CLRBF** (I2C1STAT1 bit 2): Write-1-only; clears both buffers and RXBF/TXBE flags, also clears I2C1RXIF and I2C1TXIF.
...
4. Write RxyPPS for SCL pin -> output value (0x21 for I2C1 SCL)
5. Write RxyPPS for SDA pin -> output value (0x22 for I2C1 SDA)
6. Write I2C1SCLPPS -> input pin code (0x13 for RC3)
7. Write I2C1SDAPPS -> input pin code (0x14 for RC4)
8. Optionally configure RxyI2C registers for input threshold/slew-rate
9. Enable module: I2C1CON0.EN = 1
