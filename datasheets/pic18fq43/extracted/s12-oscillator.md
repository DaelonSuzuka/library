                                                                                                              PIC18F27/47/57Q43
                                                                                            OSC - Oscillator Module (With Fail-Safe
                                                                                                                   Clock Monitor)
...
12.   OSC - Oscillator Module (With Fail-Safe Clock Monitor)
      ...
      •   Fail-Safe Clock Monitor (FSCM): Designed to detect a failure of the external clock source and
          automatically switch to an internal clock source
...
--- p201 ---
        ...
        SOSCPWR is set (SOSCPWR = 1), the oscillator operates in High-Power mode, which is ideal for
        crystal oscillators with high drive strength or high Equivalent Series Resistance (ESR).


                     Important: The SOSC module must be disabled before changing Power modes.
                     Changes to the Power mode during operation may result in undefined oscillator
                     behavior.
...
12.1.2.1.1 HFINTOSC Frequency Tuning
        ...
        to a 6-bit, two’s complement value of 0x00, which indicates that the oscillator is operating at the
...
12.3   Fail-Safe Clock Monitor (FSCM)
       The Fail-Safe Clock Monitor (FSCM) allows the device to continue operating if the external oscillator
       fails. The FSCM is applicable to all external Oscillator modes. The FSCM is enabled by setting the
       Fail-Safe Clock Monitor Enable (FCMEN) Configuration bit.
...
12.3.1 Fail-Safe Detection
        ...
        The sample clock is generated by dividing the LFINTOSC by 64. The fail detector block contains a
        latch that is set upon each falling edge of the external clock. The latch is cleared on the rising edge
...
12.3.2 Fail-Safe Operation
        When the external clock fails, the FSCM overwrites the NOSC/COSC bits to select the HFINTOSC
        and sets the Oscillator Fail Interrupt Flag (OSFIF) bit of the PIR registers. If the Oscillator Fail
        Interrupt Enable (OSFIE) bit is set, an interrupt will be generated when OSFIF is set. The frequency of
        HFINTOSC depends on the previous state of the FRQ bits and the NDIV/CDIV bits.
        Once a failure is detected, software can take steps to mitigate the effects of a failed oscillator. The
        system clock will continue to be sourced from the HFINTOSC until the external oscillator is restarted.
        Once the external source is operational, software can switch back to the external oscillator via the
        NOSC/NDIV bits.

12.3.3 Fail-Safe Condition Clearing
        The Fail-Safe condition is cleared after a device Reset, execution of a SLEEP instruction, or a change
        to the NOSC/NDIV bits. When switching to the external oscillator or PLL, the Oscillator Start-up Timer
        (OST) is restarted. While the OST is running, the device continues to from HFINTOSC. When the OST
        expires, the Fail-Safe condition is cleared after successfully switching to the external clock source.


                     Important: Software must clear the OSFIF bit before switching to the external
                     oscillator. If the Fail-Safe condition still exists, the OSFIF bit will be set again by
                     module hardware.
...
12.4         Active Clock Tuning (ACT)
             ...
             HFINTOSC over voltage and temperature.
                           ...
                           Important: Active Clock Tuning requires the use of a 32.768 kHz external
                           oscillator connected to the SOSCI/SOSCO pins.
            ...
            uses theACT
    Filename:         HFINTOSC        Frequency Tuning (TUN) bits to adjust the HFINTOSC frequency. When ACTEN is
                         Block Diagram.vsdx
    Title: clear (ACTEN = 0), the ACT feature is disabled, and user software can utilize the TUN bits to adjust
    Last Edit:      2/25/2019
    ...
    Notes:
                           ...
                           Important: When the ACT feature is enabled, the TUN bits are controlled directly
                           through module hardware and become read-only bits to user software. Writes to
                           the TUN bits when the ACT feature is enabled are ignored.
                                                                                                                            Rev. ACT B lock
                                                                                                                                 2/25/2019
...
Bit 3 – SOR Secondary Oscillator (SOSC) Ready
            ...
            0          The Secondary oscillator is not enabled, or it is not ready for use
...
Bit 1 – SFOR SFINTOSC Ready
            ...
            0          The SFINTOSC is not enabled, or it is not ready for use
