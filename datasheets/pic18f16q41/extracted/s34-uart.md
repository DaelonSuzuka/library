...
--- p519 ---
                      Important: The UxTXIF Transmitter Interrupt flag is set when the TXEN Enable bit is set
                      and the UxTXB register can accept data.
...
34.2.1.7 Transmitter Parity Modes
        When Odd or Even Parity mode is selected, all data is sent as nine bits. The first eight bits are data
...
34.2.2 UART Asynchronous Receiver
        ...
        in Figure 34-2. The data is received on the RX pin and drives the data recovery block. The data
...
34.2.2.1 Enabling the Receiver
        ...
        corresponding to the selected RX I/O pin must be set to configure the pin as an input.
                       ...
                       Important: If the RX function is on an analog pin, the corresponding ANSEL bit must be
                       cleared for the receiver to function.


34.2.2.2 Receiving Data
        Data is recovered from the bit stream by timing to the center of the bits and sampling the input
        ...
        data recovery circuit aborts character reception, without generating an error, and resumes looking
        for the falling edge of the Start bit.
...
34.2.3 Asynchronous Address Mode
       ...
       When Asynchronous Address mode is enabled, all data is transmitted and received as 9-bit
...
34.2.3.1 Address Mode Transmit
       ...
       Data is sent by writing to the UxTXB register. This transmits the written byte with the 9th bit cleared,
       which indicates that the byte is data.
...
34.2.3.2 Address Mode Receive
       ...
       and all subsequent data is ignored.
...
34.4.1 LIN Host/Client Mode
       ...
       •   ON = 1
                      ...
                      Important: The TXEN bit must be set before the Host process is received and remain set
                      while in LIN mode whether or not the Client process is a transmitter.
...
34.4.2 LIN Client Mode
        ...
        data are defined by software according to the PID code.
...
34.5    DALI Mode (Full-Featured UARTs Only)
        ...
        Manchester encoding consists of the clock and data in a single bit stream (refer to Figure 34-9). A
        high-to-low or a low-to-high transition always occurs in the middle of the bit period and may or may
        not occur at the bit period boundaries. When the consecutive bits in the bit stream are of the same
        value (i.e., consecutive ‘1’s or consecutive ‘0’s), a transition occurs at the bit boundary. However,
        when the bit value changes, there is no transition at the bit boundary. According to the standard, a
        half-bit time is typically 416.7 μs long. A double half-bit time or a single bit is typically 833.3 μs.
...
--- p534 ---
       ...
       wait time expires, being transmitted afterwards.
       If the backward frame data is written to UxTXB after the wait time has expired, it is held in the UxTXB
       register until the end of the wait time following the next forward frame. The TXMTIF bit is false when
       the backward frame data is held in the transmit shift register. Receiving a UxRXIF interrupt before
       the TXMTIF goes true indicates that the backward frame write was too late and another forward
       frame was received before sending the backward frame. The pending backward frame is flushed by
       setting the TXBE bit to prevent it from being sent after the next forward frame.
...
34.13 Collision Detection (Full-Featured UARTs Only)
       ...
       The software overhead of unloading the receive buffer of transmitted data is avoided by setting
...
34.16 UART Baud Rate Generator
      ...
      when the desired baud rate is not possible otherwise, and to improve the baud rate resolution at
...
--- p541 ---
       ...
       transferred to the UxBRG registers when complete.
                        ...
                        Important:
                        1. When both the WUE and ABDEN bits are set, the auto-baud detection will occur on the
                           byte following the Break character (see the Auto Wake-on-Break section).
                        2. It is up to the user to verify that the incoming character baud rate is within the range
                           of the selected BRG clock source. Some combinations of oscillator frequency and UART
                           baud rates are not possible.
...
34.16.3.1 Auto-Wake-Up Special Considerations
        ...
        must be of sufficient length, and be followed by a sufficient interval, to allow enough time for the
        selected oscillator to start and provide proper initialization of the UART.
        The WUE Bit
        To ensure that no actual data is lost, check the RXIDL bit to verify that a receive operation is not in
