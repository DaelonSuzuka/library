...
44.2   ZCD Logic Output
       ...
       or sink is active. The OUT bit is set when the current sink is active, and cleared when the current
...
44.3   ZCD Logic Polarity
       ...
       is set, a OUT high indicates that the current source is active, and a low output indicates that the
...
44.5   Correction for ZCPINV Offset
...
44.6   Handling VPEAK Variations
       ...
       to keep the ZCD current source and to sink below the design maximum range specified by ZC02,
       and above a reasonable minimum range, depending on the application. The compensating pull-up
       for this series resistance can be determined with the equations shown in Equation 44-5 because the
       pull-up value is independent from the peak voltage.


                   Tip: It is recommended that the maximum peak voltage be no more than six times the
